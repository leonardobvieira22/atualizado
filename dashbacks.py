...
from notification_manager import send_telegram_alert, add_notification, get_notifications, NOTIFICATION_TYPES
import uuid
import toml
import logging
...
def query_grok(prompt):
    try:
        ...
        add_notification(
            message=f"Erro ao consultar Grok: {e}",
...
def get_mark_price(symbol):
    try:
        ...
        add_notification(
            message=f"Erro ao obter preço de mercado para {symbol}: {e}",
            notification_type="ERROR",
            source="Binance API"
        )
        return None
...
def close_order_manually(signal_id, mark_price):
    ...
    if not order_idx:
        ...
        add_notification(
            message=f"Ordem {signal_id} não encontrada para fechamento manual",
            notification_type="ERROR",
            source="Dashboard"
        )
        return
    ...
    add_notification(
        message=f"Ordem {signal_id} fechada manualmente com PNL de {profit_percent:.2f}%",
...
def download_historical_data(symbol, interval='1d', lookback='30 days'):
    try:
        ...
        add_notification(
            message=f"Erro ao baixar dados históricos para {symbol}: {e}",
            notification_type="ERROR",
            source="Binance API"
        )
        return None
...
def generate_orders(robot_name, strategy_config):
    if not isinstance(robot_name, str) or not robot_name:
        ...
        add_notification(
            message=f"Nome de robô inválido: {robot_name}",
            notification_type="ERROR",
            source="Dashboard"
        )
        return None
    ...
    for timeframe in timeframes:
        ...
        if len(robot_open_orders) >= 36:
            ...
            add_notification(
                message=f"Limite de 36 ordens abertas atingido para {robot_name}",
        ...
        add_notification(
            message=f"Nova ordem simulada gerada para {robot_name}: {selected_pair} ({direction})",
            notification_type="SUCCESS",
            source=robot_name
        )
        return new_order
...
def check_alerts(df_open):
    ...
    for _, row in df_open.iterrows():
        ...
        if distance_to_tp <= 0:
            ...
            add_notification(
                message=f"Ordem {row['signal_id']} ({symbol}) fechada: TP!",
                notification_type="SUCCESS",
                source=robot_name
            )
            ...
            add_notification(
                message=f"Ordem {row['signal_id']} ({symbol}) fechada: SL!",
                notification_type="ERROR",
                source=robot_name
            )
        else:
            if distance_to_tp > 0 and distance_to_tp <= 0.5:
                ...
                add_notification(
                    message=f"Ordem {row['signal_id']} ({symbol}) a {distance_to_tp:.2f}% do TP!",
                    notification_type="WARNING",
                    source=robot_name
                )
            if distance_to_sl > 0 and distance_to_sl <= 0.5:
                ...
                add_notification(
                    message=f"Ordem {row['signal_id']} ({symbol}) a {distance_to_sl:.2f}% do SL!",
                    notification_type="ERROR",
                    source=robot_name
                )
    log_file = "bot.log"
    if os.path.exists(log_file):
        ...
        for log in recent_logs:
            if "Erro ao verificar histórico de preços" in log:
                ...
                add_notification(
                    message=f"Erro detectado no log: {log.strip()}",
                    notification_type="ERROR",
                    source="Sistema"
                )
    ...
    if len(df_closed[df_closed['resultado'].isin(['TP', 'SL'])]) < 5:
        ...
        add_notification(
            message="Modelo ML não treinado: menos de 5 ordens com TP/SL",
            notification_type="WARNING",
            source="Sistema"
        )
    return alerts
...
def reset_bot_data(reset_password):
    ...
    if reset_password != correct_password:
        add_notification(
            message="Tentativa de reset com senha incorreta",
            notification_type="ERROR",
            source="Sistema"
        )
    ...
    try:
        ...
        try:
            ...
            add_notification(
                message=f"Erro ao treinar modelo após reset: {e}",
                notification_type="ERROR",
                source="Sistema"
            )
        add_notification(
        ...
        add_notification(
            message=f"Erro ao resetar bot: {e}",
            notification_type="ERROR",
            source="Sistema"
        )
...
def validate_robot_status_and_stats():
    ...
    if os.path.exists(SINALS_FILE):
        ...
        if 'strategy_name' not in df.columns or df.empty:
            ...
            add_notification(
                message="Arquivo sinais_detalhados.csv está vazio ou sem a coluna 'strategy_name'",
                notification_type="WARNING",
                source="Sistema"
            )
            return
    ...
    if inconsistency_count > 0:
        add_notification(
            message=f"{inconsistency_count} inconsistências encontradas na validação de robôs",
            notification_type="WARNING",
            source="Sistema"
        )
...
# Barra lateral
with st.sidebar:
    st.image("static/logo.png", width=150) if os.path.exists("static/logo.png") else st.write("")
...
if page == "Visão Geral":
    ...
    with col2:
        ...
        if not prices_df.empty:
            ...
            st.info("Nenhum dado de preços disponível para exibir o gráfico.")
...
# Aba: Pesquisa Grok
    ...
    st.info("Faça consultas personalizadas sobre o mercado de criptomoedas.")
    query = st.text_input(
...
# Aba: Ordens
    ...
    st.info("Gerencie ordens simuladas (Dry Run) e visualize o histórico.")
    
    # Filtros
    col1, col2, col3 = st.columns(3)
    with col1:
        date_filter = st.date_input("Filtrar por Data", value=(datetime.now() - timedelta(days=7), datetime.now()), key="order_date_filter")
    with col2:
        direction_filter = st.multiselect("Direção", options=["LONG", "SHORT"], default=["LONG", "SHORT"], key="order_direction_filter")
    with col3:
        par_filter = st.multiselect("Par", options=["XRPUSDT", "DOGEUSDT", "TRXUSDT"], default=["XRPUSDT", "DOGEUSDT", "TRXUSDT"], key="order_par_filter")
    col4, col5, col6 = st.columns(3)
    with col4:
        robot_filter = st.multiselect("Nome do Robô", options=strategies.keys(), default=list(strategies.keys()), key="order_robot_filter")
    with col5:
        state_filter = st.multiselect("Estado", options=["Aberto", "Fechado", "Todas"], default=["Todas"], key="order_state_filter")
    with col6:
        tp_filter = st.multiselect("TP (%)", options=[0.5, 1.0, 2.0, 3.0, 5.0], default=[0.5, 1.0, 2.0, 3.0, 5.0], key="order_tp_filter")
        sl_filter = st.multiselect("SL (%)", options=[0.5, 1.0, 2.0, 3.0, 5.0], default=[0.5, 1.0, 2.0, 3.0, 5.0], key="order_sl_filter")
    ...
    st.markdown("<h2 class='section-title'>Posições Simuladas Abertas (Dry Run)</h2>", unsafe_allow_html=True)
...
<p>🌟 <strong>Score de Qualidade:</strong> {row['Quality Score']}</p>
    ...
    # Ordens fechadas
    st.markdown("<h2 class='section-title'>Histórico de Ordens Fechadas</h2>", unsafe_allow_html=True)
...
<p>🌟 <strong>Score de Qualidade:</strong> {row['Quality Score']}</p>
    ...
    # Sinais gerados
    st.markdown("<h2 class='section-title'>Sinais Gerados</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 class='section-title'>Alertas e Notificações</h2>", unsafe_allow_html=True)
    alerts = []
...
# Aba: Configurações de Estratégia
    ...
    st.markdown("<h2 class='section-title'>Indicadores Simples</h2>", unsafe_allow_html=True)
    for indicador in indicadores_simples:
        config["indicadores_ativos"][indicador] = st.checkbox(
            f"Ativar {indicador}", value=config["indicadores_ativos"].get(indicador, True), key=f"ind_simple_{indicador}"
        )
        if config["indicadores_ativos"][indicador] and indicador in ["Sentimento"]:
            config["confianca_minima"][indicador] = st.slider(
                ...
                step=0.01,
                key=f"conf_simple_{indicador}"
            )
    
    st.markdown("<h2 class='section-title'>Indicadores Compostos</h2>", unsafe_allow_html=True)
    for indicador in indicadores_compostos:
        config["indicadores_ativos"][indicador] = st.checkbox(
            f"Ativar {indicador}", value=config["indicadores_ativos"].get(indicador, True), key=f"ind_comp_{indicador}"
        )
        if config["indicadores_ativos"][indicador]:
            config["confianca_minima"][indicador] = st.slider(
                ...
                value=config["confianca_minima"].get(indicador, 0.5),
                step=0.01,
                key=f"conf_comp_{indicador}"
            )
    
    st.markdown("<h2 class='section-title'>Timeframes</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 class='section-title'>Parâmetros Gerais</h2>", unsafe_allow_html=True)
    config["tp_percent"] = st.number_input(
        "TP (%)", min_value=0.1, max_value=100.0, value=config.get("tp_percent", 2.0), step=0.1, key="tp_percent"
    )
    config["sl_percent"] = st.number_input(
        "SL (%)", min_value=0.1, max_value=100.0, value=config.get("sl_percent", 1.0), step=0.1, key="sl_percent"
    )
    config["leverage"] = st.number_input(
        "Leverage (x)", min_value=1, max_value=125, value=config.get("leverage", 22), step=1, key="leverage"
    )
    config["max_trades_simultaneos"] = st.number_input(
        "Máximo de Trades Simultâneos", min_value=1, max_value=100, value=config.get("max_trades_simultaneos", 1), step=1, key="max_trades"
    )
    config["score_tecnico_min"] = st.slider(
        "Score Técnico Mínimo", min_value=0.0, max_value=1.0, value=config.get("score_tecnico_min", 0.3), step=0.01, key="score_tecnico"
    )
    config["ml_confidence_min"] = st.slider(
        "Confiança ML Mínima", min_value=0.0, max_value=1.0, value=config.get("ml_confidence_min", 0.5), step=0.01, key="ml_confidence"
    )
    
    st.markdown("<h2 class='section-title'>Salvar Estratégia</h2>", unsafe_allow_html=True)
    strategy_name = st.text_input("Nome da Estratégia", key="strategy_name")
    if st.button("Salvar como Nova Estratégia", key="save_strategy"):
        if strategy_name:
            strategies = load_strategies()
            ...
            add_notification(
                message=f"Estratégia '{strategy_name}' salva com sucesso",
                notification_type="SUCCESS",
                source="Dashboard"
            )
        else:
            ...
            add_notification(
                message="Nome da estratégia não fornecido",
                notification_type="ERROR",
                source="Dashboard"
            )
...
# Aba: Oportunidades Perdidas
    ...
    # ... (código anterior até o início da seção indicada)
...
# Aba: Meus Robôs
    ...
    with st.expander("Resetar Todos os Dados"):
        reset_password = st.text_input("Digite a senha para reset", type="password", key="reset_password")
        if st.button("Resetar Sistema", key="reset_system"):
...
# Aba: ML Machine
    ...
    if st.button("Treinar Modelo Agora", key="train_model"):
    ...
    with st.expander("Ajustar Hiperparâmetros"):
        ...
        if st.button("Aplicar Hiperparâmetros", key="apply_hyperparams"):
            try:
