...
class GrokPeriodicCheck:
    ...
    def study_market_patterns(self, pair, close, high, low, volume, ema12, ema50, rsi, macd, signal, adx, atr):
        ...
        if close.iloc[-1] <= pattern_data["support"] * 1.005:
    ...
    def predict_movement(self, pair, close, high, low, volume, ema12, ema50, rsi, macd, signal, adx, atr):
        ...
        support = min(low[-20:]) if len(low) >= 20 else None
        resistance = max(high[-20:]) if len(high) >= 20 else None
        # Verificar canal e quebra
        highs = high[-10:]
        lows = low[-10:]
        channel = "descending" if highs.is_monotonic_decreasing and lows.is_monotonic_decreasing else "ascending" if highs.is_monotonic_increasing and lows.is_monotonic_increasing else "horizontal"
        volume_increase = (volume.iloc[-1] - volume.iloc[-2]) / volume.iloc[-2] if volume.iloc[-2] > 0 else 0
        
        # Previsão com base em quebra de canal
        if channel == "descending" and close.iloc[-1] < lows.iloc[-2] * 0.995 and volume_increase > 0.1:
            prediction_data["predicted_direction"] = "down"
            prediction_data["confidence"] = 0.7 if volume_increase > 0.2 else 0.6
            prediction_data["reason"] = (
                f"Quebra de canal descendente abaixo de {lows.iloc[-2]:.4f} com volume aumentado ({volume_increase:.2%}) "
                f"sugere continuação da queda para o próximo suporte ({support:.4f})."
            )
        elif close.iloc[-1] <= support * 1.005 and rsi.iloc[-1] > rsi.iloc[-2] and volume_increase > 0.1:
            prediction_data["predicted_direction"] = "up"
            prediction_data["confidence"] = 0.7 if volume_increase > 0.2 else 0.6
            prediction_data["reason"] = (
                f"Preço testando suporte ({support:.4f}) com divergência bullish no RSI e volume aumentado ({volume_increase:.2%}) "
                f"sugere reversão para alta em direção à resistência ({resistance:.4f})."
            )
        # Ajuste por indicadores e volatilidade
        if ema12 < ema50 and rsi.iloc[-1] < rsi.iloc[-2] and macd.iloc[-1] < signal.iloc[-1] and atr.iloc[-1] > atr.iloc[-2] * 1.1:
            prediction_data["predicted_direction"] = "down"
            prediction_data["confidence"] = max(prediction_data["confidence"], 0.7 if volume_increase > 0.1 else 0.6)
            prediction_data["reason"] += (
                f" EMA12 abaixo de EMA50, RSI caindo, MACD negativo e aumento de volatilidade (ATR) reforçam movimento descendente."
            )
        elif ema12 > ema50 and rsi.iloc[-1] > rsi.iloc[-2] and macd.iloc[-1] > signal.iloc[-1] and atr.iloc[-1] > atr.iloc[-2] * 1.1:
            prediction_data["predicted_direction"] = "up"
            prediction_data["confidence"] = max(prediction_data["confidence"], 0.7 if volume_increase > 0.1 else 0.6)
            prediction_data["reason"] += (
                f" EMA12 acima de EMA50, RSI subindo, MACD positivo e aumento de volatilidade (ATR) sugerem movimento ascendente."
            )
    ...
    async def analyze_market(self):
        ...
        for pair in ["TRXUSDT", "DOGEUSDT", "XRPUSDT"]:
            ...
            prompt = (
                f"UltraBot: Análise técnica para {pair} (15min).\n"
